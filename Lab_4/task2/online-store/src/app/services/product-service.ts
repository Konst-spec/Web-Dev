import {computed, Injectable, signal} from '@angular/core';
import { Product } from '../models/product.model'
import { CategoryModel } from '../models/category.model';

@Injectable({
  providedIn: 'root',
})
export class ProductService {
  private categories: CategoryModel[] = [
    { id: 1, name: 'Products' },
    { id: 2, name: 'Phones' },
    { id: 3, name: 'Refrigerators' },
    { id: 4, name: 'Others' }
    ];
  private products: Product[] = [
    { id: 1,
      name: 'Набережные челны туалетная бумага 1-слойный 1 шт',
      description: 'Туалетная бумага однослойная без втулки. Бумага серого цвета, плотная, мягкая. Используется в гигиенических целях.' ,
      price: 226,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h4f/h75/64532316323870.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h4f/h75/64532316323870.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/naberezhnye-chelny-tualetnaja-bumaga-1-sloinyi-1-sht-poliester-104923161/?c=750000000',
      likes: 0,
      categoryID: 4,
    },
    {
      id: 2,
      name: 'Adal молоко 3.2% 925 мл',
      description: 'Молоко "Adal" 3,2% - молоко питьевое, ультрапастеризованное, изготовлено из натурального коровьего молока, удобная упаковка и современные технологии позволяют употреблять такое молоко без предварительной обработки. ',
      price: 750,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/p71/p6d/52077494.png?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/p71/p6d/52077494.png?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/p38/p6d/52077496.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/adal-moloko-3-2-925-ml-100982670/?c=750000000',
      likes: 0,
      categoryID: 1,
    },
    {
      id: 3,
      name: 'Ластик Maped Technic Duo 511710, 1 шт',
      description: 'Ластик MAPED Technic Duo подойдет для удаления следов чернил и надписей, сделанных чернографитными или цветными карандашами. Ластик имеет прямоугольную форму бело-синего цвета, синяя часть предназначена для стирания чернил.',
      price: 10,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h56/h80/86018938044446.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h56/h80/86018938044446.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/hd6/h74/86018938077214.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/lastik-maped-technic-duo-511710-1-sht-103885134/?c=750000000',
      likes: 0,
      categoryID: 4,
    },
    {
      id: 4,
      name: 'Coca-Cola газированный напиток кола 1 л',
      description: 'Coca-Cola – один из самых популярных безалкогольных напитков в истории, а также один из наиболее узнаваемых брендов в мире.Сегодня неповторимый вкус Coca-Cola знаком миллионам людей по всему миру. Она освежает и наполняет энергией, оставляя приятное пряное послевкусие.',
      price: 665 ,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/pbb/pee/97763098.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/pbb/pee/97763098.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/h39/h36/85448954707998.png?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/coca-cola-classic-gazirovannyi-napitok-1-l-100208096/?c=750000000',
      likes: 0,
      categoryID: 1,
    },
    {
      id: 5,
      name: 'Apple iPhone 16 Pro Max 256Gb черный',
      description: 'Флагманский IPhone 16 Pro Max вобрал в себя лучшие черты современного гаджета. Это самое производительное устройство в линейке Apple iPhone с масштабным дисплеем, потрясающей производительностью и невероятной внешней эргономикой. ',
      price: 980000,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/hcf/h69/87295489343518.png?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/hcf/h69/87295489343518.png?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/h08/hd3/87295489376286.png?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/apple-iphone-16-pro-max-256gb-chernyi-123787551/?c=750000000',
      likes: 0,
      categoryID: 2,
    },
    {
      id: 6,
      name: 'SMART QUA85QX2AAJXXZ 216 см черный',
      description: 'Телевизор SMART QUA85QX2AAJXXZ — ваш идеальный спутник для развлечений, предлагающий впечатляющее качество изображения и широкие возможности подключения. С диагональю 85 см и поддержкой 4K UHD, он станет центром вашего домашнего кинотеатра.',
      price: 376948,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/p0f/pa8/14638174.jpeg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/p0f/pa8/14638174.jpeg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/smart-qua85qx2aajxxz-216-sm-chernyi-131847362/?c=750000000',
      likes: 0,
      categoryID: 4,
    },
    {
      id: 7,
      name: 'Яйцо Казгер-Кус куриное 30 шт 60-65 г',
      description: 'Свежие и качественные яйца станут отличной основой для множества вкусных блюд. Они - богатый источник протеина(белка), что так необходим для построения мышц. белок белок белок белок белок белок белок',
      price: 2349,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h10/hdd/63816604287006.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h10/hdd/63816604287006.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/jaitso-kazger-kus-kurinoe-30-sht-60-65-g-100236608/?c=750000000',
      likes: 0,
      categoryID: 1,
    },
    {
      id: 8,
      name: 'Матрас Askona Prime Mix, 180x200x20 см',
      description: 'Матрас Askona PRIME MIX – сбалансированная поддержка и комфорт\n' +
        'Askona PRIME MIX – это сочетание современных технологий и высококачественных материалов, созданное для тех, кто ценит правильную поддержку позвоночника и комфортный отдых.',
      price: 120040,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/pbc/pcf/53365260.jpg?format=gallery-medium',
      images: ['сhttps://resources.cdn-kaspi.kz/img/m/p/pbc/pcf/53365260.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pd8/pcf/53365261.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/matras-askona-prime-mix-180x200x20-sm-123622481/?c=750000000',
      likes: 0,
      categoryID: 4,
    },
    {
      id: 9,
      name: 'Алекс минеральная Saryagash газированная 1.5 л',
      description: 'Ваш собственный источник здоровья на каждый день. Alex Saryagash добывается из уникальной скважины глубиной 2000 метров - там, где вода проходит естественную минерализацию, насыщаясь полезными микроэлементами и приобретая свой особенный узнаваемый вкус и свойства.  ',
      price: 380,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/pae/p95/10441254.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/pae/p95/10441254.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/aleks-mineral-naja-saryagash-gazirovannaja-1-5-l-100211003/?c=750000000',
      likes: 0,
      categoryID: 1,
    },
    {
      id: 10,
      name: 'Майонез 3 желания провансаль 67% 700 г',
      description: 'Приготовленный по классической рецептуре майонез 3 Желания Провансаль с большим содержанием растительного масла, обладает насыщенным сбалансированным вкусом и сметанообразной консистенцией.',
      price: 1732,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h14/hed/85722698711070.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h14/hed/85722698711070.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/ha5/h52/85722698776606.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/maionez-3-zhelanija-provansal-67-700-g-100224012/?c=750000000',
      likes: 0,
      categoryID: 1,
    },
    {
      id: 11,
      name: 'Влажные детские салфетки Tomiko 120 шт',
      description: 'Гипоаллергенные влажные салфетки созданы для ухода за чувствительной кожи малыша и мамы.\n' +
        'Бесспиртовые салфетки можно использовать для детей с рождения, имеют нейтральный рн.\n' +
        'Незаменимы дома, при смене подгузника, на прогулке, в поликлинике и в путешествиях.\n' +
        'Способ применения: Откройте клапан, достаньте необходимое количество салфеток, нежно протрите кожу малыша.',
      price: 1450 ,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h87/h38/86390526541854.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h87/h38/86390526541854.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/ha4/hbc/86390526574622.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/vlazhnye-detskie-salfetki-tomiko-120-sht-113091770/?c=750000000',
      likes: 0,
      categoryID: 4,
    },
    {
      id: 12,
      name: 'Leadbros HD-442WK серый',
      description: 'Представляем вам холодильник LEADBROS HD-442WK — идеальное сочетание стиля, функциональности и энергетической эффективности. Этот холодильник с морозильником обеспечит вам надежное хранение продуктов и освежит ваш интерьер.',
      price: 304990,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/pf2/p95/85928725.png?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/pf2/p95/85928725.png?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/p0e/p96/85928726.png?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/leadbros-hd-442wk-seryi-117791413/?c=750000000',
      likes: 0,
      categoryID: 3,
    },
    {
      id: 13,
      name: 'Leadbros HD-159 черный',
      description: 'Холодильник Leadbros HD-159 — компактное и стильное решение для вашей кухни, обеспечивающее эффективное хранение продуктов и энергосбережение',
      price: 94990,
      rating: 4.9,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/p5f/p11/39085679.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/p5f/p11/39085679.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pf3/p0e/39085680.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/leadbros-hd-159-chernyi-106107741/?c=750000000',
      likes: 0,
      categoryID: 3,
    },
    {
      id: 14,
      name: 'Смартфон Apple iPhone 17 Pro 256Gb NanoSIM+eSIM оранжевый',
      description: 'Apple iPhone 17 Pro 256Gb — это воплощение инноваций и стиля, сочетающее в себе непревзойденную производительность, потрясающую камеру и яркий OLED-дисплей.',
      price: 820005,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/p18/p96/64168413.png?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/p18/p96/64168413.png?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pf3/p0e/39085680.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/apple-iphone-17-pro-256gb-nanosim-esim-oranzhevyi-145467625/?c=750000000',
      likes: 0,
      categoryID: 2,
    },
    {
      id: 15,
      name: 'Смартфон Xiaomi Redmi A5 4 ГБ/128 ГБ черный + подарок',
      description: 'Xiaomi Redmi A5 4 ГБ/128 ГБ — это универсальный смартфон для тех, кто ценит производительность и функциональность в сочетании с доступной ценой. В комплекте — наушники Xiaomi Type-C Earphones Black в подарок!',
      price: 51990,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/p20/p92/67214818.png?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/p20/p92/67214818.png?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pf3/p0e/39085680.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/xiaomi-redmi-a5-4-gb-128-gb-chernyi-podarok-137003643/?c=750000000',
      likes: 0,
      categoryID: 2,
    },
    {
      id: 16,
      name: 'Смартфон Samsung Galaxy A56 5G 8 ГБ/128 ГБ серый',
      description: 'Представляем Samsung Galaxy A56 5G — смартфон, который сочетает в себе высокую производительность, современные технологии и стильный дизайн.',
      price: 176990,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/pde/p5c/30501454.png?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/pde/p5c/30501454.png?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pf3/p0e/39085680.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/samsung-galaxy-a56-5g-8-gb-128-gb-seryi-136422081/?c=750000000',
      likes: 0,
      categoryID: 2,
    },
    {
      id: 17,
      name: 'Смартфон Xiaomi Redmi 13C 8 ГБ/256 ГБ черный',
      description: 'Представляем Xiaomi Redmi 13C — смартфон, который сочетает в себе высокую производительность, современные технологии и доступную цену. С ним вы сможете наслаждаться ярким экраном и мощной камерой в любом месте.',
      price: 59997,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h1b/h77/84526902706206.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h1b/h77/84526902706206.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pf3/p0e/39085680.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/xiaomi-redmi-13c-8-gb-256-gb-chernyi-114695323/?c=750000000',
      likes: 0,
      categoryID: 2,
    },
    {
      id: 18,
      name: 'Leadbros HD-159 черный',
      description: 'Холодильник Leadbros HD-159 — компактное и стильное решение для вашей кухни, обеспечивающее эффективное хранение продуктов и энергосбережение',
      price: 94990,
      rating: 4.9,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h1a/h9a/84567752081438.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/p5f/p11/39085679.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pf3/p0e/39085680.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/leadbros-hd-159-chernyi-106107741/?c=750000000',
      likes: 0,
      categoryID: 3,
    },
    {
      id: 19,
      name: 'Холодильник Indesit DS 4180 W белый',
      description: 'Представляем вам холодильник Indesit DS 4180 W — идеальное сочетание функциональности и стиля для вашей кухни. Этот холодильник с морозильником обеспечит надежное хранение продуктов и сохранит их свежесть на долгое время.',
      price: 174290,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/h0f/haf/63759042183198.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/h0f/haf/63759042183198.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/h1a/h9a/84567752081438.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/leadbros-hd-159-chernyi-106107741/?c=750000000',
      likes: 0,
      categoryID: 3,
    },
    {
      id: 20,
      name: 'Холодильник Muxxed HD-75 серебристый',
      description: 'Холодильник Muxxed HD-75 — компактное и эффективное решение для хранения продуктов, идеально подходящее для небольших кухонь и офисов. ',
      price: 56300,
      rating: 5,
      image: 'https://resources.cdn-kaspi.kz/img/m/p/pf8/p3b/37619564.jpg?format=gallery-medium',
      images: ['https://resources.cdn-kaspi.kz/img/m/p/pf8/p3b/37619564.jpg?format=gallery-medium', 'https://resources.cdn-kaspi.kz/img/m/p/pc0/p3b/37619566.jpg?format=gallery-medium'],
      link: 'https://kaspi.kz/shop/p/muxxed-hd-75-serebristyi-119874336/?c=750000000',
      likes: 0,
      categoryID: 3,
    },
  ];

  getProducts(){
    return this.products;
  }
  getCategories(){
    return this.categories;
  }
  getProductsByCategory(categoryID: number): Product[] {
    return this.products.filter(
      p => p.categoryID === categoryID
    );
  }

}
